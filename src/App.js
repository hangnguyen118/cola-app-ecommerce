...
//data tinh
import { productsData, categoryData, shipData, paymentData, favoriteList, cartList } from "./theme";
...
const App = () => {
  ...
  const getFavorite = async (user) => {
    ...
    //       `http://localhost:3001/api/users/${user.id}/favorite`
    //     );
  ...
  const addToCart = async (productId) => {
    ...
    //   console.error(error);
    // }
    const item = productsData.filter(item => item.id === productId);
    ...
    if(products.length!==0){
      filteredProducts = products.filter(
        (product) =>
          ...
          (category === "All" ||
            category === "" ||
            product.categoriesName === category)
      );
    } else {
  ...
  const addFavorite = async (productId) => {
    ...
    //       `http://localhost:3001/api/users/${userId}/favorite`,
    //       { productId }
    //     );
    //     if (response.status === 200) {
    ...
    <>
      ...
      <Routes>
        ...
        <Route
          ...
          element={
            user && !user.isAdmin ? (
              <CartSection userCart={userCart} removeInCart={removeInCart} />
            ) : (
              <Navigate to="/login" replace />
            )
          }
        ...
        <Route
          path="/invoicelist"
          element={<Invoicelist getInvoiceDetail={getInvoiceDetail} />}
        />
      </Routes>
      <Footer />
    </>
